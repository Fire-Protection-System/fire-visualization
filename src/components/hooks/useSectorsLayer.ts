...
// and rendered with a distinctive black border.
...
// values above ~80 should be treated as fully burned.
const LOST_BURN_LEVEL_THRESHOLD = 80;
  ...
  return useMemo(() => {
    ...
    const polygonLayer = new PolygonLayer<Sector>({
      ...
      getLineColor: (sector) => {
        ...
        // Lost sectors have a strong black border; others keep the existing red outline with reduced opacity
        return isLost ? [0, 0, 0, Math.min(255, opacity + 55)] : [255, 60, 0, opacity];
      },
      getLineWidth: () => {
        const sectorCount = sectors?.length || 1;
